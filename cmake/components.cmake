...
function(add_component name)
    ...
    if (DEFINED file_IMPL)
        add_library(${name} STATIC ${file_HEADER} ${file_IMPL})
        set(LINK_TYPE PUBLIC)
    else()
        add_library(${name} INTERFACE ${file_HEADER})
        set(LINK_TYPE INTERFACE)
    endif()

    target_compile_features(${name} ${LINK_TYPE} ${COMPONENT_DEFAULT_COMPILE_FEATURES})
    target_include_directories(${name} ${LINK_TYPE} ${COMPONENT_SOURCE_ROOT})
    set_target_properties(${name} PROPERTIES LINKER_LANGUAGE CXX)

    if(DEFINED file_TEST)
        add_executable(${name}_test ${file_TEST} src/boost/ut.hpp)
        target_link_libraries(${name}_test ${LINK_TYPE} ${name})
